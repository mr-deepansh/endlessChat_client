...
import { aj, createPostLimiter, loginLimiter, searchLimiter, handleArcjetResponse } from '@/lib/arcjet';
...
export const useArcjetProtection = () => {
  ...
  const protectRequest = useCallback(async (
  ...
  const protectLogin = useCallback(async (ip?: string) => {
